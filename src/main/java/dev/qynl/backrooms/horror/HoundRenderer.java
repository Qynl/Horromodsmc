...
public class HoundRenderer extends EntityRenderer<HoundEntity> {
    ...
    public void render(HoundEntity entity, float yaw, float tickDelta, MatrixStack matrices,
        ...
        float h = 1.0f;
        int a = 245;
        vc.vertex(m, -w, 0, 0).color(255, 255, 255, a).texture(0, 1).light(light).next();
        vc.vertex(m, -w, h, 0).color(255, 255, 255, a).texture(0, 0).light(light).next();
        vc.vertex(m, w, h, 0).color(255, 255, 255, a).texture(1, 0).light(light).next();
        vc.vertex(m, w, 0, 0).color(255, 255, 255, a).texture(1, 1).light(light).next();
